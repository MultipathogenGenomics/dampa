...
def print_usage(module, version):
    if module == None:
        ...
        print('makeprobeswinput - probe panel design using a general purpose incremental strategy with input probes')
        ...
        print(f'\nProbeTools MakeProbes v{version}')
        print('https://github.com/KevinKuchinski/ProbeTools\n')
        print('Usage: probetools incrementalprobes -t <target seqs> -b <batch size> -o <path to output directory>/<output name> -x <existing probes> [<optional args>]')
        print('Required arguments:')
        print(' -t : path to target sequences in FASTA file')
        print(' -b : number of probes in each batch (min=1)')
        print(' -o : path to output directory and name to append to output files')
        print(' -x : existing probes file')
        print('Optional arguments:')
        print(' -m : max number of probes to add to panel (default=MAX, min=1)')
        print(' -c : target for 10th percentile of probe coverage (default=90, min=1, max=100)')
        print(' -k : length of probes to generate (default=120, min=32)')
...
def cluster_kmers(out_path, name, targets_path, k, cluster_id, step, max_degen, prev_probes_path, num_probes, threads):
    ...
    kmerno = enum_kmers(targets_path, kmers_path, k, step, max_degen)
    if kmerno == 0:
...
def get_data_from_inputprobes(out_path,targets_path,existingprobes,name,threads,min_id, min_capture_length):
    ...
    with open(existingprobes, "r") as file:
        panel_size = sum(1 for line in file if line.startswith(">"))
...
def cluster_kmers_with_VSEARCH(kmers_path, centroids_path, cluster_id, threads):
    ...
    finished_process = subprocess.call(terminal_command, shell=True)#, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def add_BLAST_results_to_capture_data(blast_path, capture_data, min_id, min_length):
    ...
    with open(blast_path, 'r') as input_file:
        for line in input_file:
            ...
            if float(line[2]) >= min_id and float(line[3]) >= min_length:
                probe=line[0]
                if probe not in mappedprobes:
                    mappedprobes.append(probe)
                target = line[1].strip()
    ...
    print(f'MAPPED PROBES:{len(mappedprobes)}')
